...
public class SkuServiceImpl implements SkuService {
    ...
    public void decrCount(String username) {
        //1 获取购物车数据
        List<Object> values = stringRedisTemplate.boundHashOps(CART + username).values();
        ...
        for (Object value : values) {
            ...
            if(i<=0){
                throw new RuntimeException("库存不足，减库存失败。skuid"+orderItem.getSkuId()+"。orderId"+orderItem.getOrderId());
            }
        }
    }
