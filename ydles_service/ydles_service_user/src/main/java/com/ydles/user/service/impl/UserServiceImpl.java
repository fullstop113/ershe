...
public class UserServiceImpl implements UserService {
    ...
    public int updateUserPoint(Task task) {
        System.out.println("开始加积分了！");

        //7 数据库中 积分日志表 这个任务是否做过
        ...
        //10 user添加积分
        int i = userMapper.updateUserPoint(userId, point);
        if(i<=0){
            return 0;
        }
        ...
        pointLog1.setUserId(userId);
        pointLog1.setPoint(point);
        int j = pointLogMapper.insertSelective(pointLog1);
        if(j<=0){
            return 0;
        }
